...
def amen_invert(A, nswp = 20, x0 = None, eps = 1e-10, rmax = 1000, max_full = 500, kickrank = 4, kick2 = 0, trunc_norm = 'res', local_iterations = 40, resets = 2, verbose = True, preconditioner = None):
    ...
    if x0!=None:
        x0 = torchtt.TT([tn.reshape(c, [c.shape[0], -1, c.shape[-1]]) for c in x0.cores])
        
    device = A.cores[0].device
    dtype = A.cores[0].dtype 
    b = torchtt.rank1TT([tn.eye(n, device=device, dtype=dtype).flatten() for n in N])
    cores = []
...
#     """
#     w = oe.contract('lsr,smnS,LSR,rnkR->lmkL',Phi_left,coreA,Phi_right,core)
#     return w
# 
# class LinearOp():
...
# 
# def amen_invert(a, b, nswp = 22, x0 = None, eps = 1e-10,rmax = 100, max_full = 500, kickrank = 4, kick2 = 0, trunc_norm = 'res', local_iterations = 40, resets = 2, verbose = True, preconditioner = None):
#     
#     
#  
...
#     if x0 == None:
#         rx = [1] + (d-1)*[2] + [1]
#         x_cores = [ tn.ones([rx[k],N[k], N[k],rx[k+1]], dtype = dtype, device = device) for k in range(d)]
#     else:
#         x = x0
...
#     z_cores = [ tn.randn([rz[k],N[k], N[k], rz[k+1]], dtype = dtype, device = device) for k in range(d)]
...
#     Phi_next = oe.contract('lsr,lML,sMNS,rMR->LSR',Phi_now,core_left,core_A,core_right)
...
#     Phi = oe.contract('BR,bmnB,rmnR->br',Phi_now,core_b,core)
#     return Phi
# 
...
#     Phi_next = oe.contract('br,bmnB,rmnR->BR',Phi_now,core_rhs,core)
#     return Phi_next
# 